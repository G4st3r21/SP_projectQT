...
class TableBlockConstructor:
    ...
    def fill_CYR_GP_sample(self, indicator, event_id, event, main_event_id, main_event, subprogram_id, subprogram,
        ...
        path_test = self.fill_CYR_sample(2, '../tableTemplates/CYR-GP.xlsx', indicator_rf, task_id, task, cyr_id, cyr, indicator_vo, gosprogram, response_obj, '../test.xlsx')
        self.fill_GP_sample(5, path_test, indicator, event_id, event, main_event_id, main_event, subprogram_id, subprogram,
    ...
    def fill_GP_GP_sample(self, indicator, event_id, event, main_event_id, main_event, subprogram_id, subprogram,
                       type, unit, years, plan_marks, fact_marks, year_comment, comment, indicator_1, event_id_1, event_1,
                        main_event_id_1, main_event_1, subprogram_id_1, subprogram_1, type_1, unit_1, years_1, plan_marks_1,
                            fact_marks_1, year_comment_1, comment_1, path):
        path_test = self.fill_GP_sample(2, '../tableTemplates/GP-GP.xlsx', indicator, event_id, event, main_event_id, main_event,
                                        subprogram_id, subprogram, type, unit, years, plan_marks, fact_marks, year_comment, comment, '../test1.xlsx')
        self.fill_GP_sample(6, path_test, indicator_1, event_id_1, event_1, main_event_id_1, main_event_1, subprogram_id_1,
                            subprogram_1, type_1, unit_1, years_1, plan_marks_1, fact_marks_1, year_comment_1, comment_1, path)
        os.remove(path_test)
    ...
    def fill_GP_sample(self, i, sample, indicator, event_id, event, main_event_id, main_event, subprogram_id, subprogram,
                       type, unit, years, plan_marks, fact_marks, year_comment, comment, path):
        file = sample
        book = openpyxl.open(file)
        sheet = book.active
        sheet.cell(3, i + 1).value = indicator
        sheet.cell(4, i).value = 'Мероприятие №' + event_id if event_id is not Null else 'Мероприятие'
        sheet.row_dimensions[4].height = 40
        sheet.cell(4, i + 1).value = event if event is not Null else '-'
        sheet.cell(5, i).value = 'Основное мероприятие №' + main_event_id if main_event_id is not Null else 'Основное мероприятие'
        sheet.cell(5, i + 1).value = main_event if main_event is not Null else '-'
    ...
    def fill_CYR_sample(self, i, sample, indicator_rf, task_id, task, cyr_id, cyr, indicator_vo, gosprogram, response_obj, path):
        file = sample
        book = openpyxl.open(file)
        sheet = book.active
...
test.fill_CYR_GP_sample('indicator', 'event_id', 'event', 'main_event_id', 'main_event', 'subprogram_id', 'subprogram',
                       'type', 'unit', 'years', 'plan_marks', 'fact_marks', 'year_comment', 'comment',
                        'indicator_rf', 'task_id', 'task', 'cyr_id', 'cyr', 'indicator_vo', 'gosprogram', 'response_obj', '../CYR-GP.xlsx')
